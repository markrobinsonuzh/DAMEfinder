...
allele. Since the ASM score here depends on SNPs, we can see what SNPs are involved in the ASM calculation at each CpG position:
...
If you think plotting all the samples separately is difficult to see, you can use the function `dame_track_median` to summarize:
```{r, dt6}
dame_track_median(dame = dame,
           derASM = derASM,
           ASM = ASMscore)
