...
#' @param ... Arguments passed to \code{\link{loessByCluster}}. Only used if \code{smooth} = TRUE. 
#'
#' @return A vector of t-Statistics within the \code{RangedSummarizedExperiment}.
...
get_tstats <- function(sa, design, method="ls", smooth = TRUE, maxGap=300, coef=2, verbose=TRUE, ...) {
  ...
  S4Vectors::mcols(sa)$cluster <- bumphunter::clusterMaker(chr = as.character(GenomeInfoDb::seqnames(sa)),
                                                           pos = midpt, 
                                                           maxGap = maxGap,
                                                           assumeSorted = TRUE)
